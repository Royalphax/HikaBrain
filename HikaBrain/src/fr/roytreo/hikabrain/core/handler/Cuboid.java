...
/**
 * Thanks to JBoss925 for having created and shared this amazing class <3 Some
 * modification comes from me and the class getWalls() too.
 * 
 * @author JBoss925
 */
...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public Cuboid(Location l1, Location l2) {
		if (l1.getWorld().getName().equals(l2.getWorld().getName())) {
			...
			throw new IllegalArgumentException("The two location mentionned are not in the same world.");
		}
	}
	...
	public ArrayList<Block> getWalls() {
		ArrayList<Block> output = new ArrayList<>(iterator().iterateBlocks());
		...
		for (Block block : cubo.iterator().iterateBlocks())
			output.remove(block);
